...
// USER: Replace the config below with your Firebase Project settings
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (auth) {
        ...
        onAuthStateChanged(auth, async (user) => {
            if (user) {
                console.log('User joined session:', user.email);
                ...
                if (docSnap.exists()) {
                    // Existing User -> Load Profile and Skip to App
    ...
    if (btnGoogle && auth) {
        ...
        btnGoogle.addEventListener('click', async () => {
            try {
                showToast('🔄 Opening Secure Google Login...');
    ...
    // Step 1: Standard ID Authentication (Simplified for Firebase)
    if (authForm) {
        authForm.addEventListener('submit', (e) => {
            e.preventDefault();
            showToast('ℹ️ Real-time DB requires Google Login for security.');
        });
    }

    // Auto-fill Remembered Email - REMOVED, Firebase handles session

    // Step 2: Profile Setup & Database Storage
    if (profileForm) {
        profileForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            userData.name = document.getElementById('setup-name').value.trim();
            userData.dept = document.getElementById('setup-dept').value.trim();
            userData.year = document.getElementById('setup-year').value;
    ...
    function finalizeLogin() {
        ...
        document.getElementById('widget-name').textContent = userData.name;
        document.getElementById('widget-dept').textContent = userData.dept;
        document.getElementById('widget-year').textContent = userData.year;
        document.querySelector('.avatar-circle').textContent = userData.name.charAt(0).toUpperCase();
...
function openProfileEdit() {
    showToast('Editing capability coming soon in v10.0!');
}
